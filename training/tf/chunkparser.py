...
class ChunkParser:
    def __init__(self, chunkdatasrc, shuffle_size=1, sample=1, buffer_size=1, batch_size=256, workers=None):
        """
            ...
            decode. Preferred format to use on disk. 2176 bytes long on a 19x19.
            ...
            have a fast way to unpack bit vectors. 7950 bytes long on a 19x19.
        """
        # Build probility reflection tables. The last element is 'pass' and is identity mapped.
        self.prob_reflection_table = [[remap_vertex(vertex, sym) for vertex in range(BOARD_SQUARES)]+[BOARD_SQUARES] for sym in range(8)]
        # Build full 16-plane reflection tables.
        self.full_reflection_table = [
            np.array([remap_vertex(vertex, sym) + p * BOARD_SQUARES for p in range(16) for vertex in range(BOARD_SQUARES) ])
                for sym in range(8) ]
    ...
    def convert_v1_to_v2(self, text_item):
        """
            ...
            winner,...
        """
        # We start by building a list of 16 planes, each being a BOARD_SQUARES element array
        # of type np.uint8
        planes = []
        ...
        if np.any(np.isnan(probabilities)):
            ...
            return False, None
        assert len(probabilities) == BOARD_SQUARES+1

        probs = probabilities.tobytes()
        assert(len(probs) == (BOARD_SQUARES+1) * 4)
...
class ChunkParserTest(unittest.TestCase):
    def generate_fake_pos(self):
        ...
        planes = [np.random.randint(2, size=BOARD_SQUARES).tolist() for plane in range(16)]
    ...
    def test_parsing(self):
        ...
        batch = ( np.reshape(np.frombuffer(data[0], dtype=np.uint8), (batch_size, 18, BOARD_SQUARES)).tolist(),
                  np.reshape(np.frombuffer(data[1], dtype=np.float32), (batch_size, BOARD_SQUARES+1)).tolist(),
        ...
        for i in range(batch_size):
            ...
            for symmetry in range(8):
                ...
                sym_planes = [ [ plane[remap_vertex(vertex, symmetry)] for vertex in range(BOARD_SQUARES) ] for plane in planes ]
                sym_probs = [ probs[remap_vertex(vertex, symmetry)] for vertex in range(BOARD_SQUARES)] + [probs[BOARD_SQUARES]]
